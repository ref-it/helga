<div class="p-8 flex flex-col gap-6 min-h-full">
    ...
    <div class="grid md:grid-cols-2 gap-6">
        ...
        <flux:field>
            ...
            <flux:input wire:model.live="start" type="datetime-local" value="{{old('start', $shift->start)}}" />
            <flux:error name="start" />
        </flux:field>
        <flux:field>
            <flux:label>{{__("shift.endDesc")}}</flux:label>
            <flux:input wire:model="end" type="datetime-local" value="{{old('end', $shift->end)}}" />
            <flux:error name="end" />
        </flux:field>
        <flux:field>
            <flux:label>{{__("shift.team_sizeDesc")}}</flux:label>
            <flux:input wire:model="team_size" type="number" value="{{old('team_size', $shift->team_size)}}" />
